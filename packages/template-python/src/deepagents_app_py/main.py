...
用法:
  deepagents-app-py [command] [args] [flags]
...
示例:
  deepagents-app-py                            # ACP 服务器
  deepagents-app-py chat --debug               # REPL 调试模式
  deepagents-app-py ask "hello world"          # 单次问答
  deepagents-app-py run prompt.md              # 从文件运行
  deepagents-app-py graph                      # 输出节点关系图 JSON
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if command == "chat":
            from deepagents_app_py.surfaces.cli.repl import start_repl
        ...
        if command == "ask":
            if not command_arg:
                ...
                print(
                    'Usage: deepagents-app-py ask "your question"',
                    file=sys.stderr,
                )
                return 1
            from deepagents_app_py.surfaces.cli.one_shot import run_one_shot
        ...
        if command == "run":
            if not command_arg:
                ...
                print(
                    "Usage: deepagents-app-py run <prompt-file>",
                    file=sys.stderr,
                )
                return 1
            from deepagents_app_py.surfaces.cli.one_shot import run_prompt_file

            return run_prompt_file(command_arg, cli_options)

        if command == "graph":
            from deepagents_app_py.runtime.code_graph import (
                generate_code_graph,
                write_code_graph,
            )
