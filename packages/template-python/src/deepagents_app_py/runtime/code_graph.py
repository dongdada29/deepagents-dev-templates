...
def generate_code_graph(  # noqa: C901
    ...
    src_dir = pkg_root / "src" / "deepagents_app_py"
    root = str(root_path)
    ...
    _add_if_exists({
        ...
        "kind": "entrypoint",
        "path": "src/deepagents_app_py/main.py",
        "editable": "protected",
    })
    ...
    for node_def in [
        ...
        _add_if_exists({
            ...
            "label": node_def[1],
            "kind": "runtime",
            "path": f"src/deepagents_app_py/{node_def[2]}",
            "editable": "protected",
        })
    ...
    if mw_dir.exists():
        for mw_file in sorted(mw_dir.glob("*.py")):
            ...
            _add_if_exists({
                ...
                "label": label,
                "kind": "middleware",
                "path": f"src/deepagents_app_py/runtime/middleware/{mw_file.name}",
                "editable": "protected",
            })
    ...
    if tools_dir.exists():
        for tool_file in sorted(tools_dir.glob("*.py")):
            ...
            _add_if_exists({
                ...
                "label": label,
                "kind": "tool",
                "path": f"src/deepagents_app_py/app/tools/{tool_file.name}",
                "editable": "ai-user",
            })
